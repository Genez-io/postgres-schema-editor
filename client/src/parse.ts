import { TableColumn, TableColumns, TableSchema, RefObj } from './Types.js'
//import { property } from 'cypres/types/lsodash';
...
function createPrimaryKey(primaryKeyName:string, primaryKeyTableName:string) {
  const primaryKey:PrimaryKeyModel = new PrimaryKeyModel();
...
function parseTableName(name:string) {
  if (name.charAt(name.length - 1) === '(') {
    name = parseSQLServerName(name, null);
  }
  return name;
};
...
export default function parseSql(text:string) {
