...
import { RowsOfData } from '../../Types.js';
...
export default function DataTableNodeColumn({
  ...
  const onSave = async () => {
    ...
    if (schemaStore.hasOwnProperty(id!)) {
      ...
      window.alert(
        `The table ${changes.tableName} does not have a Primary Key, cannot update without one`
      );
      console.error(
        `The table ${changes.tableName} does not have a Primary Key, cannot update without one`
      );
    ...
    await fetch(import.meta.env.VITE_API_URL + `/api/sql/${dbCredentials.db_type}/updateRow`, {
      method: 'PATCH',
      headers: {
        ...
        <td className="dark:text-[#f8f4eb]" key={`${id}-${ind}`}>
          {mode === 'edit' ? (
            <input
              ...
              value={tempData[element] as string | number | undefined}
              onChange={(e) => {
