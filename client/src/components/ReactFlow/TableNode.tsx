...
export default function TableNode({ data }: any) {
  ...
  const { setInputModalState } = useSettingsStore((state:any) => state);
  ...
  for (let i = 0; i < data.edges.length; i++) {
    if (data.edges[i].source === tableName) {
      ...
      let columnNumberSource =
        columnData.findIndex((obj: any) => obj.Name === data.edges[i].sourceHandle) + 1;
    ...
    if (data.edges[i].target === tableName) {
      ...
      let columnNumberTarget =
        columnData.findIndex((obj: any) => obj.Name === data.edges[i].targetHandle) + 1;
